...
mod tests {
    ...
    fn test_weighted_velocity_and_cell_dist_to_term_x_zero() {
        ...
        assert_eq!(weighted_velocity_and_cell_dist_to_term(Vec2::new(1.0, 1.0), Vec2::new(0.0, 1.0)), zm);
    ...
    fn test_weighted_velocity_and_cell_dist_to_term_y_zero() {
        ...
        assert_eq!(weighted_velocity_and_cell_dist_to_term(Vec2::new(1.0, 1.0), Vec2::new(1.0, 0.0)), zm);
        assert_eq!(0.0, zm.determinant());
    }

    #[test]
    fn test_weighted_velocity_and_cell_dist_to_term() {
        let zm = Mat2::from_cols(Vec2::new(0.22 * 2.0, 0.77 * 2.0), Vec2::new(0.22 * -1.0, 0.77 * -1.0));
        assert_eq!(weighted_velocity_and_cell_dist_to_term(Vec2::new(0.22, 0.77), Vec2::new(2.0, -1.0)), zm);
        assert_eq!(0.0, zm.determinant());
    ...
    fn test_update_cells_iteration() {
        ...
        world.insert_resource(Grid { cells: vec!(Cell { velocity: Vec2::ZERO, mass: 0.0 }; TEST_GRID_WIDTH * TEST_GRID_WIDTH), width: TEST_GRID_WIDTH, dt: TEST_DT });
        update_stage.add_system(update_cells);
        // add particle to world
        let particle_id = world.spawn().insert_bundle(
    ...
    fn test_particles_to_grid_iteration() {
        ...
        let mut gr = Grid { cells: vec!(Cell { velocity: Vec2::ZERO, mass: 0.0 }; TEST_GRID_WIDTH * TEST_GRID_WIDTH), width: TEST_GRID_WIDTH, dt: TEST_DT };
    ...
    fn test_grid_to_particles_iteration() {
        ...
        let particle_1_id = world.spawn().insert_bundle(
        ...
        let particle_2_id = world.spawn().insert_bundle(
        ...
        assert_eq!(true, approx_equal(particle_1_position.unwrap().0.x, 5.2497, 3));
        assert_eq!(true, approx_equal(particle_1_position.unwrap().0.y, 5.3496, 3));
        assert_eq!(true, approx_equal(particle_1_velocity.unwrap().0.x, 0.497, 3));
        assert_eq!(true, approx_equal(particle_1_velocity.unwrap().0.y, 0.497, 3));
        assert_eq!(particle_1_mass.unwrap().0, 1.06);
        assert_eq!(true, approx_equal(particle_1_affine_momentum.unwrap().0.x_axis.x, 0.71, 3));
        assert_eq!(true, approx_equal(particle_1_affine_momentum.unwrap().0.x_axis.y, 0.71, 3));
        assert_eq!(true, approx_equal(particle_1_affine_momentum.unwrap().0.y_axis.x, 0.3976, 3));
        assert_eq!(true, approx_equal(particle_1_affine_momentum.unwrap().0.y_axis.y, 0.3976, 3));


        assert_eq!(true, approx_equal(particle_2_position.unwrap().0.x, 6.692, 3));
        assert_eq!(true, approx_equal(particle_2_position.unwrap().0.y, 5.992, 3));
        assert_eq!(true, approx_equal(particle_2_velocity.unwrap().0.x, -0.69135, 3));
        assert_eq!(true, approx_equal(particle_2_velocity.unwrap().0.y, 0.00704, 3));
        assert_eq!(particle_2_mass.unwrap().0, 1.23);
        assert_eq!(true, approx_equal(particle_2_affine_momentum.unwrap().0.x_axis.x, -0.557, 3));
        assert_eq!(true, approx_equal(particle_2_affine_momentum.unwrap().0.x_axis.y, -0.557, 3));
        assert_eq!(true, approx_equal(particle_2_affine_momentum.unwrap().0.y_axis.x, -1.47264, 3));
        assert_eq!(true, approx_equal(particle_2_affine_momentum.unwrap().0.y_axis.y, -1.47264, 3));
    }
    ...
    fn test_update_grid() {
        ...
        gr.cells[border_cell_index] = Cell{ velocity: Vec2::new(2.2, -2.4), mass: 1.17171717 };

        // add middle cell with mass and velocity
        let middle_cell_index = gr.index_at(5, 5);
        gr.cells[middle_cell_index] = Cell{ velocity: Vec2::new(3.7333, -1.111), mass: 3.333 };

        // apply grid update
        gr.update();

        // border cell should have updated velocity and -y velocity cancelled
        assert_eq!(true, approx_equal(gr.cells[border_cell_index].velocity.x, 1.8757, 5));
        assert_eq!(true, approx_equal(gr.cells[border_cell_index].velocity.y, 0.0, 5));
        assert_eq!(gr.cells[border_cell_index].mass, 1.17171717);

        // middle cell should have updated velocity
        assert_eq!(true, approx_equal(gr.cells[middle_cell_index].velocity.x, 1.120102, 5));
        assert_eq!(true, approx_equal(gr.cells[middle_cell_index].velocity.y, -0.36333, 5));
